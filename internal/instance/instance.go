...
func WorkflowYAML(version, sha256, cron string) string {
	...
	return fmt.Sprintf(`# Generated by gh-star-charts %[1]s. Re-running "gh star-charts init" or "gh star-charts add" upgrades it.
...
func WriteReadme(r *Repo, m *manifest.Manifest, version string) error {
	...
	if len(paused) > 0 {
		...
		// The instance is named explicitly: the reader's default instance may
		// be a different repository than the one this README lives in.
		fmt.Fprintf(&b, "These charts are paused. Their files and URLs keep serving the last recorded state. Resume one with `gh star-charts add owner/repo --charts-repo %s`.\n\n", r.FullName)
